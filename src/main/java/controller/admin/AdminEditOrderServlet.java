...
/**
 * Servlet for editing order status in the admin panel.
 */
...
public class AdminEditOrderServlet extends HttpServlet {
	...
	/**
	 * Handles GET requests to display the edit order form.
	 */
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			request.setAttribute("showModal", "edit-order-modal");
			...
			try {
				...
				session.setAttribute("error", "Failed to render the edit order page.");
				redirectWithState(request, response);
			}
		} catch (SQLException e) {
			session.setAttribute("error", "Unable to load edit order form due to a database error.");
			redirectWithState(request, response);
	...
	/**
	 * Handles POST requests to update order status.
	 */
	@Override
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (orderStatus == null || orderStatus.trim().isEmpty()) {
				session.setAttribute("error", "Order status is required.");
				redirectWithState(request, response);
				return;
			}
			...
			// Log status update
			try {
				auditLogDAO.create(new AuditLog(userId, "UPDATE_ORDER",
						"Admin updated order ID: " + orderId + " to status: " + orderStatus));
			...
			session.setAttribute("error", "Unable to update order status due to a database error.");
			try {
				auditLogDAO.create(new AuditLog(userId, "ERROR_UPDATE_ORDER", "Database error: " + e.getMessage()));
			} catch (SQLException ignored) {
				// Suppress audit logging errors
			}
			redirectWithState(request, response);
			...
			session.setAttribute("error", e.getMessage());
			try {
				auditLogDAO.create(new AuditLog(userId, "ERROR_UPDATE_ORDER", "Invalid input: " + e.getMessage()));
			} catch (SQLException ignored) {
				// Suppress audit logging errors
			}
			redirectWithState(request, response);
